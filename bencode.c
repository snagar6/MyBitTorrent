...
static void _be_dump(be_node *node)
{
	size_t i;
	...
	switch (node->type) {
		case BE_STR:
			if(strcmp(str,"announce")==0){
				...
				memcpy(meta_data.announceUrl,node->val.s,MAXLEN);
			}
			else if(strcmp(str,"name")==0){				
				memset(meta_data.file_name,'\0',MAXLEN);
				memcpy(meta_data.file_name,node->val.s,MAXLEN);
			}
			else if(strcmp(str,"pieces")==0){
				memset(meta_data.pieces,'\0',MAXLEN);
				memcpy(meta_data.pieces,node->val.s,MAXLEN);
				meta_data.numPieces=be_str_len(node);	
			}
		...
		case BE_INT:
			if(strcmp(str,"piece length")==0){
				meta_data.pieceLength=node->val.i;
			}	
			else if(strcmp(str,"length")==0){
				meta_data.file_length=node->val.i;
			}
			break;

		case BE_LIST:
			for (i = 0; node->val.l[i]; ++i);
			//	_be_dump(node->val.l[i]);
		...
		case BE_DICT:
			...
			for (i = 0; node->val.d[i].val; ++i) {
				sprintf(str,"%s",node->val.d[i].key);
				_be_dump(node->val.d[i].val);
			}
			break;
	}
...
static void _tracker_dump(be_node *node)
{
	size_t i;

	switch (node->type) {
		case BE_STR:
				//memset(tracker_data.peers,'\0',MAXLEN);
				//memcpy(tracker_data.peers,node->val.s,MAXLEN);
		...
		case BE_INT:
			...
			if(strcmp(str,"complete")==0){
				tracker_data.complete=node->val.i;
			}	
			else if(strcmp(str,"downloaded")==0){
				tracker_data.downloaded=node->val.i;
			}
			else if(strcmp(str,"incomplete")==0){
				tracker_data.downloaded=node->val.i;
			}
			else if(strcmp(str,"interval")==0){
				tracker_data.interval=node->val.i;
			}
			else if(strcmp(str,"min interval")==0){
				tracker_data.minInterval=node->val.i;
			}
			else if(strcmp(str,"peers")==0){
				tracker_data.peers=node->val.i;
			}
			break;
		...
		case BE_DICT:
			...
			for (i = 0; node->val.d[i].val; ++i) {
				sprintf(str,"%s",node->val.d[i].key);
				_tracker_dump(node->val.d[i].val);
			}
			break;
	}
...
void tracker_dump(be_node *node)
{
	_tracker_dump(node);
}
